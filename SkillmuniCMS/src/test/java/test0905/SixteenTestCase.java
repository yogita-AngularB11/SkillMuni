...
public class SixteenTestCase {
	public static void main(String[] args) throws Exception {
		...
		for (int i = 0; i < categoryElements.size(); i++) {
			...
			try {
				By allCardsLocator = By
						.xpath("//div[contains(@class, 'cards-container')]/div[contains(@class, 'card')]");
				...
				for (int c = 0; c < cards.size(); c++) {
					...
					try {
						String title = card.findElement(By.xpath(".//p[contains(@class,'card-title')]")).getText()
								.trim();
						...
						wait.until(
								ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@class='drag-track']")));
						...
						List<WebElement> briefs = driver
								.findElements(By.xpath("//div[contains(@class,'scroll-item')]"));
