...
public class SeventeenTestCase3 {
					...
					for (int j = 0; j < briefs.size(); j++) {
						...
						if (hasVideo) {
							try {
								...
								// Check for error inside iframe (e.g. "This video is private")
								String errorMessage = (String) js.executeScript(
										"var err = document.querySelector('.ytp-error'); return err ? err.textContent : '';");

								if (!errorMessage.isEmpty()) {
									System.out.println("\u001B[31m🚫 YouTube error: " + errorMessage + "\u001B[0m");
								} else {
									// Try to play video
									...
									if (state == 1) {
										...
										System.out.println("\u001B[31m⚠️ No video tag found.\u001B[0m");
									}
								}
								driver.switchTo().defaultContent();
								...
								System.out.println(
										"\u001B[31m⚠️ Could not verify video state: " + e.getMessage() + "\u001B[0m");
								try {
									driver.switchTo().defaultContent();
								} catch (Exception ex) {
									System.out.println("\u001B[31m⚠️ Error returning from iframe: " + ex.getMessage()
											+ "\u001B[0m");
								}
							}
						}
